        ...
        <main className="min-h-screen bg-[#050505] text-[#ffffff] p-12 md:p-32 animate-elegant font-sans">
            <div className="max-w-7xl mx-auto space-y-48">
                <header className="space-y-8 relative">
                    ...
                    <h1 className="text-8xl font-light tracking-tighter text-white font-serif italic uppercase">Bazar</h1>
                    ...
                    <div className="grid md:grid-cols-3 gap-12">
                        {data.tintaItems?.map(t => (
                            <div key={t.id} className="royal-card p-12 text-center group bg-gradient-to-b from-white/[0.02] to-transparent hover:border-gold/30">
                                <span className="text-6xl mb-8 block drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] group-hover:drop-shadow-[0_0_20px_rgba(212,175,55,0.6)] transition-all">💧</span>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-20">
                        {data.libros.map(b => (
                            <div key={b.id} className="royal-card p-12 group space-y-12">
                                <div className="h-[400px] w-full flex items-center justify-center text-9xl opacity-30 group-hover:opacity-100 group-hover:scale-105 transition-all duration-1000 select-none bg-gradient-to-b from-white/[0.02] to-transparent">
                                    {b.imagen}
                                </div>
                                <div className="space-y-6">
                                    <h3 className="text-4xl font-serif italic text-white/90 group-hover:text-gold transition-all duration-700">{b.titulo}</h3>
                                    <div className="flex justify-between items-center pt-8 border-t border-white/5">
                                        <div className="space-y-2">
                                            <p className="text-[10px] text-gray-500 tracking-[0.3em] font-sans uppercase">Legado: {b.autor}</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
                        {data.merch.map(m => (
                            <div key={m.id} className="royal-card p-10 text-center space-y-10 group border-transparent hover:border-gold/20">
                                <div className="text-6xl opacity-20 group-hover:opacity-100 group-hover:rotate-12 transition-all duration-700">{m.imagen}</div>
